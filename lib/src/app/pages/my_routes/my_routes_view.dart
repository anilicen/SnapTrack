...
import 'package:st/src/app/pages/my_routes/my_routes_controller.dart';
...
class MyRoutesView extends View {
  @override
  State<StatefulWidget> createState() {
    return _MyRoutesViewState(
      MyRoutesController(
        DataRouteRepository(),
      ),
    );
...
class _MyRoutesViewState extends ViewState<MyRoutesView, MyRoutesController> {
  _MyRoutesViewState(MyRoutesController controller) : super(controller);
  ...
  Widget get view {
    return Scaffold(
      body: Column(
        children: [
          STAppBarWithBack('My Routes'),
          ControlledWidgetBuilder<MyRoutesController>(
            builder: (context, controller) {
              return controller.myRoutes == null
                  ? Expanded(
                      child: Center(
                  ...
                  : Expanded(
                      child: SingleChildScrollView(
                        ...
                        child: Container(
                          ...
                          child: Column(
                            children: [
                              for (Route route in controller.myRoutes!) _MyRouteWidget(route: route),
                            ],
                          ),
                        ),
                      ),
...
class _MyRouteWidget extends StatelessWidget {
  const _MyRouteWidget({
    required this.route,
  });
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: ControlledWidgetBuilder<MyRoutesController>(builder: (context, controller) {
        return Column(
          children: [
            SizedBox(height: 10),
            Slidable(
              endActionPane: ActionPane(
                ...
                children: [
                  ...
                  SlidableAction(
                    onPressed: (context) {
                      controller.removeMyRoute(route);
                    },
